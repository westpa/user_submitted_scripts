...
class WEDriver:
    ...
    def process_config(self):
        ...
        if (not numpy.issubdtype(type(self.largest_allowed_weight), numpy.floating)) or (
            not numpy.issubdtype(type(self.smallest_allowed_weight), numpy.floating)
        ):
            try:
    ...
    def assign(self, segments, initializing=False, endprop=False, recycling=False):
        ...
        all_pcoords = numpy.empty((2,len(segments), self.system.pcoord_ndim+1), dtype=self.system.pcoord_dtype)
        if recycling:
            for iseg, segment in enumerate(segments):
                all_pcoords[0,iseg] = numpy.append(segment.pcoord[0,:],-1)
                all_pcoords[1,iseg] = numpy.append(segment.pcoord[-1,:],-1)
        else:
            for iseg, segment in enumerate(segments):
                all_pcoords[0,iseg] = numpy.append(segment.pcoord[0,:],segment.weight)
                all_pcoords[1,iseg] = numpy.append(segment.pcoord[-1,:],segment.weight)
        if endprop:
            if exists('binbounds.npz'):
                temp_npz = numpy.load('binbounds.npz')
                temp_arraylist = []
                for iarray in temp_npz.files:
                    temp_arraylist.append(iarray)
                numpy.savez('binbounds.npz', (*temp_arraylist), all_pcoords[1,:,:])
            else:
                numpy.savez('binbounds.npz', all_pcoords[1,:,:])
    ...
    def _split_by_weight(self, ibin):
        ...
        to_split = segments[
            numpy.logical_and(weights > self.weight_split_threshold * ideal_weight, weights / 2 > float(self.smallest_allowed_weight))
        ]
    ...
    def _adjust_count(self, ibin):
        ...
        while len(bin) < target_count:
            ...
            segments = numpy.array(sorted(bin, key=weight_getter), dtype=numpy.object_)
            weights = numpy.array(list(map(weight_getter, segments)))
